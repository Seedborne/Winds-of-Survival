...
func _ready() -> void:
	$GlassSprite.rotation = randf_range(deg_to_rad(-60), deg_to_rad(0))
	$GlassSprite.flip_v = randf_range(0, 1) < 0.5
...
func _on_glass_area_body_entered(body: Node) -> void:
	if body is Player and is_available:
		near_resource = true
		if Globals.tutorial_mode_on:
			$InteractLabel.show()

func _on_glass_area_body_exited(body: Node) -> void:
	if body is Player:
		near_resource = false
...
func collect_resource() -> void:
	...
	$GlassArea/GlassCollision.disabled = true
	$StaticBody2D/CollisionShape2D.disabled = true
	var random_amount = randi_range(1, 2)
	if Globals.add_item_to_inventory("glass", random_amount):
		print("Collected ", random_amount, " glass.")
	else:
		print("Inventory full, couldn't collect glass.")
	is_available = false
...
func respawn_resource() -> void:
	is_available = true
	$GlassSprite.visible = true
	$GlassArea/GlassCollision.disabled = false
	$StaticBody2D/CollisionShape2D.disabled = false
	print("Glass respawned")
	var attempts = 0
	...
	if attempts == max_attempts:
		print("Warning: Could not find a valid position for Glass after ", max_attempts, " attempts.")
