...
@export var upgrade_levels = [
	{"level": "broken", "next": "glass", "cost": {"glass": 2}, "sprite": "res://assets/Brokenwindow.png"},
	{"level": "glass", "next": "wood", "cost": {"wood": 2}, "sprite": "res://assets/window.png"},
	{"level": "wood", "next": "", "cost": null, "sprite": "res://assets/Windowreinforced.png"}
]
...
func _ready():
	current_level_index = Globals.window1_index
	update_sprite()
...
func deduct_resources():
	...
	Globals.window1_index += 1
	if current_level_index == 1:
		Globals.shack_strength += 5
		$Window1Label.text = "Upgrade window 
		for 2 wood"
...
func _on_window_1_area_2d_body_entered(body: Node) -> void:
	if body is Player:
		player_in_area = true
		$Window1Dot.hide()
		if current_level_index < upgrade_levels.size() - 1:
...
func _on_window_1_area_2d_body_exited(body: Node) -> void:
	if body is Player:
